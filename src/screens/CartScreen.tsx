...
export const CartScreen: React.FC<ScreenProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="flex flex-col w-full pb-32 pt-16 bg-[#070F1E] text-[#FAF9F6] min-h-screen font-sans relative">
      {/* Top Header */}
      <section className="px-5 pt-4 pb-3 border-b border-[#C29C6D]/20 bg-[#0B1528]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="font-['Libre_Caslon_Text',serif] text-2xl font-bold text-[#FAF9F6]">
              Your Fabri<span className="text-[#D4AF37]">Q</span> Cart
            </h1>
          </div>
          <button
            onClick={() => setIsWhatsAppOpen(true)}
            className="w-10 h-10 rounded-2xl bg-[#070F1E] text-[#E5C07B] border border-[#C29C6D]/40 flex items-center justify-center shadow-md hover:border-[#D4AF37] transition-colors cursor-pointer"
            title="Instant Concierge Chat"
          >
      ...
      {cartItems.length === 0 ? (
        ...
        <section className="px-5 py-12 text-center flex flex-col items-center justify-center my-auto">
          <div className="w-24 h-24 rounded-full bg-[#0B1528] border-2 border-[#C29C6D]/50 flex items-center justify-center text-[#E5C07B] shadow-2xl mb-4">
            <span className="material-symbols-outlined text-[48px]">shopping_cart</span>
          </div>
          <div className="inline-flex items-center gap-2 bg-[#0B1528] border border-[#C29C6D]/40 px-3 py-1 rounded-full mb-3 text-[#E5C07B] text-xs font-bold">
            <span className="material-symbols-outlined text-[16px]">shopping_cart</span>
            <span>0 Items • Cart Value: ₹0</span>
          </div>
          <h2 className="font-['Libre_Caslon_Text',serif] text-2xl font-bold text-[#FAF9F6] mb-1">
            Your Cart is Empty (₹0)
          </h2>
          <p className="text-xs text-slate-300 max-w-xs mb-6 font-medium">
            Discover our haute couture dry cleaning, eco wash & iron, and bespoke garment care menu.
          </p>
          <button
            onClick={() => onNavigate('service-catalog')}
            className="px-6 min-h-[44px] bg-gradient-to-r from-[#D4AF37] to-[#C29C6D] text-[#0B1528] rounded-2xl text-xs font-black uppercase tracking-wider shadow-lg hover:opacity-90 transition-all cursor-pointer flex items-center justify-center gap-2"
          >
        ...
        <div className="space-y-5 mt-4">
          ...
          <section className="px-5">
            <div className="flex items-center justify-between mb-2.5">
              <h3 className="text-xs font-black text-[#E5C07B] uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="space-y-3">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="bg-[#0B1528] p-3.5 rounded-2xl border border-[#C29C6D]/30 shadow-md flex items-center gap-3.5 hover:border-[#D4AF37] transition-all"
                >
                  <img
                    src={item.image}
                    alt={item.garmentName}
                    className="w-16 h-16 rounded-xl object-cover shrink-0 border border-[#C29C6D]/20"
                  />
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-sm text-[#FAF9F6] truncate">{item.garmentName}</h4>
                    <div className="mt-1">
                      <select
                        value={item.service}
                        onChange={(e) => updateCartItemService(item.id, e.target.value as ServiceType)}
                        className="bg-[#070F1E] hover:bg-[#050A14] border border-[#C29C6D]/40 text-[#E5C07B] text-[10px] font-black px-2 py-1 rounded-lg focus:outline-none focus:border-[#D4AF37] cursor-pointer"
                      >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="flex items-center bg-[#070F1E] rounded-xl p-1 border border-[#C29C6D]/40">
                      <button
                        onClick={() => updateCartQty(item.id, -1)}
                        className="w-6 h-6 rounded-lg bg-[#0B1528] text-[#FAF9F6] flex items-center justify-center font-bold text-xs hover:bg-[#122240] transition-colors cursor-pointer"
                      >
                        -
                      </button>
                      <span className="w-6 text-center text-xs font-bold text-[#FAF9F6]">
                        {item.qty}
                      </span>
                      <button
                        onClick={() => updateCartQty(item.id, 1)}
                        className="w-6 h-6 rounded-lg bg-[#D4AF37] text-[#0B1528] flex items-center justify-center font-black text-xs hover:bg-[#E5C07B] transition-colors cursor-pointer"
                      >
                        +
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="w-8 h-8 rounded-xl bg-rose-950/40 text-rose-400 hover:bg-rose-900/60 border border-rose-800/40 flex items-center justify-center transition-colors cursor-pointer shrink-0"
                      title="Remove Item"
                    >
          ...
          <section className="px-5">
            <h4 className="text-[10px] font-extrabold text-[#E5C07B] uppercase tracking-widest mb-2">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
              {[
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-[#0B1528] text-slate-200 text-xs font-bold border border-[#C29C6D]/30 hover:border-[#D4AF37] hover:bg-[#0E1B33] whitespace-nowrap transition-all cursor-pointer shadow-xs flex items-center gap-1 shrink-0"
                >
                  <span>{svc.name}</span>
                </button>
              ))}
          ...
          <section className="px-5">
            <div className="bg-[#0B1528] text-white rounded-2xl p-4 border border-[#C29C6D]/40 shadow-md">
              <div className="flex items-center gap-2 mb-2">
                <span className="material-symbols-outlined text-[#D4AF37] text-[18px]">psychology</span>
                <span className="text-[10px] font-extrabold text-[#E5C07B] uppercase tracking-widest">
                  FABRIQ AI CARE RECOMMENDATIONS
                </span>
              </div>

              <div className="space-y-2">
                <label className="flex items-center justify-between p-2.5 bg-[#070F1E] rounded-xl border border-[#C29C6D]/30 cursor-pointer hover:border-[#D4AF37] transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-[#FAF9F6]">Add Nano Stain Shield Protection</p>
                      <p className="text-[10px] text-slate-400">Repels liquids & spills on silks & suiting</p>
                    </div>
                  </div>
                  <span className="text-xs font-bold text-[#E5C07B]">+₹49</span>
                </label>
                ...
                <label className="flex items-center justify-between p-2.5 bg-[#070F1E] rounded-xl border border-[#C29C6D]/30 cursor-pointer hover:border-[#D4AF37] transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-[#FAF9F6]">Express 4-Hour Valet Delivery</p>
                      <p className="text-[10px] text-slate-400">Priority processing at Jubilee Hills Atelier</p>
                    </div>
                  </div>
                  <span className="text-xs font-bold text-[#E5C07B]">+₹99</span>
                </label>
                ...
                <label className="flex items-center justify-between p-2.5 bg-[#070F1E] rounded-xl border border-[#C29C6D]/30 cursor-pointer hover:border-[#D4AF37] transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-[#FAF9F6]">Ozone Anti-Microbial Sanitization</p>
                      <p className="text-[10px] text-slate-400">Deep pathogen neutralization</p>
                    </div>
                  </div>
                  <span className="text-xs font-bold text-[#E5C07B]">+₹39</span>
                </label>
              </div>
            </div>
          </section>
          ...
          <section className="px-5">
            <div className="bg-[#0B1528] border border-[#C29C6D]/40 rounded-2xl p-3.5 flex items-start gap-3">
              <span className="material-symbols-outlined text-[#E5C07B] text-[22px] shrink-0 mt-0.5">local_shipping</span>
              <div className="text-xs space-y-1">
                <span className="font-extrabold text-[#E5C07B] uppercase tracking-wider block text-[10px]">
                  🚚 FREE PICKUP & DROP POLICY
                </span>
                <p className="text-slate-300 text-[11px] leading-snug font-medium">
                  • <strong className="text-white">Free 5 km Radius</strong>: On Cart value <strong className="text-[#E5C07B]">₹799 & above</strong><br />
                  • <strong className="text-white">Free 10 km Radius</strong>: On Cart value <strong className="text-[#E5C07B]">₹2,599 & above</strong>
                </p>
                {subtotal < 799 ? (
                  <p className="text-[10px] text-[#E5C07B] font-bold mt-1 bg-[#070F1E] border border-[#C29C6D]/30 px-2 py-0.5 rounded w-fit">
                    💡 Add ₹{799 - subtotal} more to unlock Free 5 km Pickup!
                  </p>
                ) : subtotal < 2599 ? (
                  <p className="text-[10px] text-emerald-400 font-bold mt-1 bg-[#070F1E] border border-emerald-500/40 px-2 py-0.5 rounded w-fit">
          ...
          <section className="px-5">
            <div className="bg-[#0B1528] p-4 rounded-2xl border border-[#C29C6D]/30 shadow-md space-y-3">
              <div className="flex items-center justify-between border-b border-[#C29C6D]/20 pb-2">
                <span className="text-xs font-bold text-[#FAF9F6] uppercase tracking-wider flex items-center gap-1.5">
                  <span className="material-symbols-outlined text-[#E5C07B] text-[18px]">location_on</span>
                  <span>Doorstep Pickup Address</span>
                </span>
                <button
                  onClick={() => setIsEditingAddress(!isEditingAddress)}
                  className="text-xs font-bold text-[#E5C07B] hover:underline cursor-pointer"
                >
              ...
              {isEditingAddress ? (
                <input
                  ...
                  className="w-full bg-[#070F1E] border border-[#C29C6D]/50 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-[#D4AF37]"
                />
              ) : (
                <p className="text-xs font-medium text-slate-300 leading-snug">{pickupAddress}</p>
              )}

              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#C29C6D]/20">
                <div>
                  <label className="text-[10px] font-bold text-[#E5C07B] uppercase block mb-1">
                    Pickup Date
                  </label>
                  <select
                    value={pickupDate}
                    onChange={(e) => setPickupDate(e.target.value)}
                    className="w-full bg-[#070F1E] border border-[#C29C6D]/40 rounded-xl p-2 text-xs font-bold text-white focus:outline-none focus:border-[#D4AF37]"
                  >
          ...
          <section className="px-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Promo Code */}
            <div className="bg-[#0B1528] p-3.5 rounded-2xl border border-[#C29C6D]/30 shadow-md">
              <span className="text-[10px] font-extrabold text-[#E5C07B] uppercase tracking-widest block mb-1.5">
                PROMO CODE / COUPON
              </span>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#070F1E] border border-[#C29C6D]/40 rounded-xl px-2.5 py-1.5 text-xs font-bold uppercase text-white focus:outline-none focus:border-[#D4AF37]"
                />
                <button
                  onClick={handleApplyCoupon}
                  className="px-3 py-1.5 bg-gradient-to-r from-[#D4AF37] to-[#C29C6D] text-[#0B1528] font-black text-xs rounded-xl transition-all hover:opacity-90 cursor-pointer"
                >
                  Apply
                </button>
              </div>
              {appliedCoupon && (
                <p className="text-[10px] font-bold text-emerald-400 mt-1.5 flex items-center gap-1">
            ...
            <div className="bg-[#0B1528] p-3.5 rounded-2xl border border-[#C29C6D]/30 shadow-md">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="bg-[#070F1E] border border-[#C29C6D]/40 text-[#E5C07B] text-[9px] font-bold px-2 py-0.5 rounded">
          ...
          <section className="px-5">
            <div className="bg-[#0B1528] p-4 rounded-2xl border border-[#C29C6D]/30 shadow-md space-y-2">
              <h3 className="text-xs font-extrabold text-[#E5C07B] uppercase tracking-wider pb-2 border-b border-[#C29C6D]/20">
              ...
              <div className="flex justify-between text-sm font-black text-white pt-2 border-t border-[#C29C6D]/20">
                <span>Total Amount</span>
                <span className="text-[#E5C07B] text-base font-extrabold">₹{grandTotal}</span>
              </div>
            </div>
          </section>

          {/* SECTION 7: PAYMENT METHODS */}
          <section className="px-5 w-full min-w-0">
            <h3 className="text-xs font-extrabold text-[#E5C07B] uppercase tracking-wider mb-2">
              Select Payment Method
            </h3>
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 w-full min-w-0">
              {[
                ...
                <button
                  ...
                  className={`p-2.5 min-h-[44px] rounded-2xl border text-center transition-all cursor-pointer flex flex-col items-center justify-center gap-1 ${
                    selectedPayment === pm.id
                      ? 'bg-[#0E1B33] text-[#E5C07B] border-[#D4AF37] shadow-md ring-2 ring-[#D4AF37]/30'
          ...
          <section className="px-5 pt-2 w-full min-w-0">
            <button
              onClick={handleCheckout}
              className="w-full min-h-[48px] py-4 bg-gradient-to-r from-[#D4AF37] to-[#C29C6D] text-[#0B1528] font-black text-xs sm:text-sm uppercase tracking-wider rounded-2xl shadow-xl hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-98"
            >
      ...
      <div className="fixed bottom-20 right-4 z-40">
        <button
          onClick={() => setIsWhatsAppOpen(true)}
          className="w-11 h-11 rounded-full bg-[#0B1528] text-[#E5C07B] flex items-center justify-center shadow-xl hover:border-[#D4AF37] transition-all hover:scale-105 cursor-pointer border-2 border-[#C29C6D] ring-2 ring-[#C29C6D]/20"
