...
export const StoreManagerDashboard: React.FC<StoreManagerDashboardProps> = ({ onNavigate }) => {
  ...
  const [activeFilter, setActiveFilter] = useState<'intake' | 'manual_order' | 'pricing' | 'reports' | 'machines'>('intake');
        ...
        <div className="bg-slate-900 border border-[#9E7B4F]/40 rounded-3xl p-5 text-white shadow-md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <img
                src={profile?.avatarUrl || 'https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=200&auto=format&fit=crop'}
                alt={profile?.name}
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-extrabold text-amber-300 uppercase tracking-widest block">
                    BRANCH PANEL & COUNTER ERP
                  </span>
                  {/* Branch Open/Close Status Toggle */}
                  <button
                    onClick={() => setIsStoreOpen(!isStoreOpen)}
                    className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider flex items-center gap-1 cursor-pointer transition-colors ${
                ...
                <h1 className="font-['Libre_Caslon_Text',serif] text-xl font-bold text-white mt-0.5">
                  {profile?.name || 'Rajesh Kumar'}
                </h1>
                <p className="text-xs text-slate-300">
                  {profile?.storeLocation || 'Jubilee Hills Flagship Store #101'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-amber-400 hover:bg-amber-300 text-slate-950 px-3 py-1.5 rounded-xl text-xs font-extrabold flex items-center gap-1 cursor-pointer shadow-sm"
              >
                <span className="material-symbols-outlined text-[16px]">add_circle</span>
                <span>Manual Offline Order</span>
              </button>
              <ExportDataButton variant="compact" label="Export Reports" />
              <button
          ...
          <div className="grid grid-cols-4 gap-2 pt-3 border-t border-slate-800 text-center">
            <div className="bg-slate-800/80 p-2.5 rounded-2xl border border-slate-700">
              <span className="text-[9px] font-bold text-slate-400 uppercase block">Intake Today</span>
              <span className="font-bold text-amber-300 text-base">142 Pcs</span>
            </div>
            <div className="bg-slate-800/80 p-2.5 rounded-2xl border border-slate-700">
              <span className="text-[9px] font-bold text-slate-400 uppercase block">Offline Cash Orders</span>
              <span className="font-bold text-emerald-400 text-base">18 Orders</span>
            </div>
            <div className="bg-slate-800/80 p-2.5 rounded-2xl border border-slate-700">
              <span className="text-[9px] font-bold text-slate-400 uppercase block">Pickup Scheduled</span>
              <span className="font-bold text-sky-400 text-base">12 Pickups</span>
            </div>
            <div className="bg-slate-800/80 p-2.5 rounded-2xl border border-slate-700">
              <span className="text-[9px] font-bold text-slate-400 uppercase block">Ready for Delivery</span>
              <span className="font-bold text-white text-base">24 Orders</span>
            </div>
          </div>
        </div>
      </section>
