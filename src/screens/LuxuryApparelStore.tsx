...
export const LuxuryApparelStore: React.FC<ScreenProps> = ({ onNavigate }) => {
  ...
  const [genderFilter, setGenderFilter] = useState<'All' | 'Men' | 'Women' | 'Kids' | 'Others'>('All');
  ...
  const genderTabs = ['All', 'Men', 'Women', 'Kids', 'Others'] as const;
  ...
  return (
    <div className="flex flex-col w-full pb-28 pt-16 bg-[#070F1E] text-[#FAF9F6] min-h-screen font-sans selection:bg-[#C29C6D] selection:text-black">
      
      {/* Luxury Hero Store Banner */}
      <section className="px-5 pt-5 pb-3 max-w-7xl mx-auto w-full">
        <div className="bg-[#0B1528] rounded-3xl p-6 border-2 border-[#C29C6D]/40 shadow-2xl relative overflow-hidden">
          <div className="relative z-10 space-y-2">
            ...
            <h1 className="font-['Libre_Caslon_Text',serif] text-2xl sm:text-3xl font-bold text-[#FAF9F6] leading-tight">
              FabriQ Luxury Cloth Store
            </h1>
            <p className="text-xs text-slate-300 max-w-xl font-normal leading-relaxed">
              Premium clothing & lifestyle retail. Ready-to-wear wardrobe crafted from Giza cotton, Japanese selvedge denim, Italian calfskin, and pure silk.
            </p>

            <div className="flex flex-wrap gap-4 pt-2 text-[11px] text-[#E5C07B] font-bold">
      ...
      <section className="px-5 my-3 space-y-3 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
          {genderTabs.map((g) => (
            <button
              ...
              className={`px-4 py-2 min-h-[44px] rounded-2xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
                genderFilter === g
            ...
            >
              {g === 'All' ? '✨ ALL COLLECTIONS' : g.toUpperCase()}
            </button>
          ))}
        </div>
        ...
        <div className="relative">
          <span className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-[#E5C07B] text-[20px]">
            search
          </span>
          <input
            type="text"
            ...
            className="w-full bg-[#0B1528] border border-[#C29C6D]/30 focus:border-[#D4AF37] text-white text-xs pl-10 pr-10 py-3 rounded-2xl outline-none placeholder:text-slate-500 transition-colors shadow-inner font-sans"
          />
          {searchQuery && (
            <button
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`btn-press px-4 py-2 rounded-2xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer min-h-[44px] ${
                  isActive
                    ? 'bg-[#0E1B33] text-[#E5C07B] border border-[#D4AF37] shadow-md font-extrabold ring-1 ring-[#D4AF37]/30'
                    : 'bg-[#0B1528] text-slate-300 border border-[#C29C6D]/30 hover:border-[#C29C6D]'
                }`}
              >
                {cat === 'Shirts' && '👔 '}
                {cat === 'T-Shirts' && '👕 '}
      ...
      <section className="px-5 my-2 max-w-7xl mx-auto w-full">
        {isLoading ? (
          <SkeletonLoader variant="product-grid" count={6} />
        ) : filteredProducts.length === 0 ? (
          <div className="bg-[#0B1528] rounded-3xl p-8 text-center border border-[#C29C6D]/30 my-6">
            <span className="material-symbols-outlined text-4xl text-[#E5C07B] mb-2 block">
              search_off
            </span>
            ...
            <button
              ...
              className="mt-4 px-4 py-2 bg-gradient-to-r from-[#D4AF37] to-[#C29C6D] text-[#0B1528] font-bold text-xs rounded-xl cursor-pointer hover:opacity-90"
            >
              Reset Filters
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="bg-[#0B1528] rounded-3xl border border-[#C29C6D]/30 hover:border-[#D4AF37] transition-all overflow-hidden shadow-lg flex flex-col justify-between group"
                >
                  {/* Image & Favorite Overlay */}
                  <div className="relative h-64 w-full bg-[#070F1E] overflow-hidden">
                    <FabriQBrandedImage
                    ...
                    {/* Badge */}
                    {product.badge && (
                      <span className="absolute top-3 left-3 z-10 bg-[#E5C07B] text-[#0B1528] font-black text-[9px] px-2.5 py-1 rounded-lg uppercase tracking-wider shadow-md">
                        {product.badge}
                      </span>
                    )}
                    ...
                    <button
                      ...
                      className={`absolute top-3 right-3 z-10 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 cursor-pointer backdrop-blur-md shadow-xl transform active:scale-125 ${
                        isFav
                          ? 'bg-rose-950/90 border-2 border-rose-500 text-rose-500 ring-4 ring-rose-500/30'
                          : 'bg-[#070F1E]/80 border border-[#C29C6D]/30 text-slate-300 hover:text-white hover:border-[#D4AF37]'
                      }`}
                    ...
                    {/* Category Label */}
                    <span className="absolute bottom-3 left-3 text-[10px] font-extrabold text-[#E5C07B] uppercase tracking-widest font-sans">
                  ...
                  <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between">
                        <h3 className="font-['Libre_Caslon_Text',serif] font-bold text-base text-[#FAF9F6] group-hover:text-[#E5C07B] transition-colors leading-snug">
                          {product.name}
                        </h3>
                      </div>
                      ...
                      <div className="mt-3 space-y-1">
                        <span className="text-[9px] font-bold text-[#E5C07B] uppercase tracking-wider block font-sans">
                          SELECT SIZE:
                        </span>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {product.sizes.map((size) => (
                            <button
                              ...
                              className={`px-2.5 py-1 rounded-lg text-[10px] font-extrabold transition-all cursor-pointer min-h-[32px] ${
                                selectedSize === size
                                  ? 'bg-[#0E1B33] text-[#E5C07B] border border-[#D4AF37] font-black shadow-xs ring-1 ring-[#D4AF37]/30'
                                  : 'bg-[#070F1E] text-slate-400 border border-[#C29C6D]/30 hover:border-[#D4AF37]'
                              }`}
                            >
                              {size}
                            </button>
                    ...
                    {/* Pricing & Actions */}
                    <div className="pt-3 border-t border-[#C29C6D]/20 space-y-2">
                      <div className="flex items-baseline justify-between">
                        <div className="flex items-baseline gap-2">
                          <span className="font-['Libre_Caslon_Text',serif] font-extrabold text-lg text-[#FAF9F6]">
                        ...
                        <div className="flex items-center gap-1 text-[11px] text-[#E5C07B] font-bold">
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1 font-sans">
                        <button
                          ...
                          className="btn-press py-2.5 px-3 bg-[#070F1E] hover:bg-[#0E1B33] text-[#E5C07B] border border-[#C29C6D]/40 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer min-h-[44px]"
                        >
                        ...
                        <button
                          ...
                          className="btn-press py-2.5 px-3 bg-gradient-to-r from-[#D4AF37] to-[#C29C6D] hover:opacity-95 text-[#0B1528] rounded-xl font-black text-xs shadow-md flex items-center justify-center gap-1 transition-all cursor-pointer min-h-[44px]"
                        >
                          <span>Buy Now</span>
