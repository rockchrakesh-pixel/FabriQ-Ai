...
export const LiveOrderTracking: React.FC<ScreenProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-[#FAFAFC] relative">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 text-white backdrop-blur-xl rounded-t-3xl shadow-2xl p-5 pb-24 border-t border-amber-400/50 max-w-2xl mx-auto">
        <div className="w-12 h-1 bg-amber-400/60 rounded-full mx-auto mb-3" />
        ...
        {selectedAgent ? (
          <div className="bg-slate-950 p-4 rounded-2xl border border-amber-400/40 space-y-2 mb-4">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider">
                {selectedAgent.roleTitle}
              </span>
              <span className="text-xs font-bold text-amber-300">{selectedAgent.rating}</span>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
            {FLEET_AGENTS.map((ag) => (
              <div
                ...
                className="bg-slate-950 p-3 rounded-2xl border border-slate-800 hover:border-amber-400/60 transition-colors cursor-pointer flex items-center gap-3"
              >
                <div
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="py-3 rounded-2xl bg-slate-800 text-white text-xs font-bold uppercase tracking-wider hover:bg-slate-700 transition-colors flex items-center justify-center gap-2 cursor-pointer border border-slate-700"
          >
          ...
          <button
            onClick={() => onNavigate('home')}
            className="py-3 rounded-2xl bg-amber-400 text-slate-950 text-xs font-extrabold uppercase tracking-wider shadow-md hover:bg-amber-300 transition-colors flex items-center justify-center gap-2 cursor-pointer fabriq-glow"
          >
