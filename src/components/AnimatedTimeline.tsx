...
export const AnimatedTimeline: React.FC = () => {
  ...
  return (
    <div className="bg-[#0f172a] text-white rounded-3xl p-5 md:p-6 border border-amber-400/40 shadow-2xl relative my-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          ...
          <span className="text-xs font-bold text-amber-300 bg-amber-400/20 px-3 py-1 rounded-full border border-amber-400/40 inline-block">
            Stage 3 of 6 • Active
          </span>
        </div>
      </div>
      ...
      <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden mb-6 relative">
        <motion.div
          className="bg-gradient-to-r from-[#9E7B4F] via-amber-400 to-emerald-400 h-full rounded-full relative"
          initial={{ width: '0%' }}
          animate={{ width: '50%' }}
      ...
      <div className="relative pl-6 space-y-6 before:absolute before:left-[15px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gradient-to-b before:from-emerald-500 before:via-amber-400 before:to-slate-800">
        {stages.map((stage) => {
            ...
            >
              ...
              <div
                ...
                className={`absolute -left-[31px] top-1 w-8 h-8 rounded-full flex items-center justify-center cursor-pointer transition-all z-10 ${
                  isCompleted
                    ? 'bg-emerald-500 text-slate-950 ring-4 ring-emerald-950 shadow-lg'
                    : isActive
                    ? 'bg-amber-400 text-slate-950 ring-4 ring-amber-950/80 shadow-2xl scale-110'
                    : 'bg-slate-800 text-slate-400 ring-4 ring-slate-950 border border-slate-700'
                }`}
              >
              ...
              <div
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer ${
                  isActive
                    ? 'bg-slate-900 border-amber-400/80 shadow-xl ring-1 ring-amber-400/40'
                    : isCompleted
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className={`text-xs font-bold font-sans ${isActive ? 'text-amber-300' : 'text-white'}`}>
                        {stage.title}
                      </h4>
                      {isActive && (
