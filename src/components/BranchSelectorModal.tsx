  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-amber-400/60 flex flex-col max-h-[92vh]">
        ...
        {/* GPS Quick Detect & Google Location Search Banner */}
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 px-5 py-3 flex flex-col gap-2.5 border-b border-amber-400/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[20px]">my_location</span>
              <span className="text-xs font-bold">Google & GPS Location Finder</span>
            </div>
            <button
          ...
          <form
            onSubmit={(e) => {
              ...
              if (locationSearchInput) {
                setCustomLocation(locationSearchInput);
              }
            }}
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Type city, area, street or pincode (e.g. Bowenpally, Suchitra, Kompally, London)..."
                value={locationSearchInput}
                onChange={(e) => setLocationSearchInput(e.target.value)}
                className="w-full bg-slate-950/90 text-white placeholder-slate-400 pl-8 pr-3 py-1.5 text-xs rounded-xl border border-amber-400/30 focus:outline-none focus:border-amber-300 font-sans"
              />
            </div>
            <button
              type="submit"
              className="bg-slate-950 hover:bg-slate-900 text-white px-3 py-1.5 rounded-xl text-xs font-bold shrink-0 border border-amber-400/30 cursor-pointer"
            >
              Set Location
            </button>
          </form>
          ...
          <div className="bg-slate-950/90 text-amber-200 p-2 rounded-xl text-[10px] font-mono flex items-center justify-between border border-amber-400/30">
            <span className="truncate max-w-[280px]">
              {gpsDetectedName || `📍 Selected Store: ${activeBranch.name} (${activeBranch.area ? activeBranch.area + ', ' : ''}${activeBranch.city})`}
            </span>
            <span className="text-emerald-400 font-bold shrink-0">0.8 km Store Distance</span>
          </div>
        </div>
