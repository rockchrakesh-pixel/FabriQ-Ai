...
export const OnlineBillingModal: React.FC<OnlineBillingModalProps> = ({
  ...
  const [couponCode, setCouponCode] = useState(() => {
    return localStorage.getItem('fabriq_active_promo') || 'SILK20';
  });
  ...
  // Dynamic discount calculation based on selected coupon code
  let discountAmount = 0;
  let discountLabel = 'DISCOUNT ACTIVE';
  const cleanCode = appliedOfferCode.trim().toUpperCase();

  if (cleanCode === 'SILK20') {
    discountAmount = Math.round(subtotal * 0.20);
    discountLabel = '20% SILK CARE SAVINGS';
  } else if (cleanCode === 'VIPLUX') {
    discountAmount = Math.min(150, subtotal);
    discountLabel = '₹150 EXPRESS VALET FREE';
  } else if (cleanCode === 'BESPOKE15') {
    discountAmount = Math.round(subtotal * 0.15);
    discountLabel = '15% SUIT CARE SAVINGS';
  } else if (cleanCode === 'HERITAGEGIFT') {
    discountAmount = Math.min(100, subtotal);
    discountLabel = '₹100 BOUTIQUE GIFT';
  } else if (cleanCode === 'FABRIQ5OFF' || cleanCode === 'FABRIQ5STAR') {
    discountAmount = Math.round(subtotal * 0.05);
    discountLabel = '5% MEMBER SAVINGS';
  } else if (cleanCode) {
    discountAmount = Math.round(subtotal * 0.10);
    discountLabel = '10% PROMO SAVINGS';
  }

  const taxes = Math.round((subtotal - discountAmount) * 0.05); // 5% GST
  const grandTotal = Math.max(0, subtotal - discountAmount + taxes);
  ...
  const handleApplyCoupon = (codeToApply?: string) => {
    ...
    localStorage.setItem('fabriq_active_promo', code);
  };
          ...
          <div className="bg-amber-50/70 p-3.5 rounded-2xl border border-amber-200 space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-900 flex items-center gap-1">
                ...
                Privilege Offers & Promo Vouchers
              </span>
              {discountAmount > 0 && (
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Enter SILK20 / VIPLUX / BESPOKE15"
              ...
              <button
                ...
                className="bg-amber-500 hover:bg-amber-600 text-slate-950 px-3.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer shadow-2xs"
              >
                Apply
              </button>
            </div>

            {/* Quick Available Offer Badges */}
            <div className="space-y-1 pt-1">
              <span className="text-[9px] font-extrabold text-[#9E7B4F] uppercase tracking-wider block">
                Tap to Apply Active Offers:
              </span>
              <div className="flex flex-wrap gap-1.5">
                {[
                  { code: 'SILK20', label: 'SILK20 (20% Off)' },
                  { code: 'VIPLUX', label: 'VIPLUX (Valet Free)' },
                  { code: 'BESPOKE15', label: 'BESPOKE15 (15% Off)' },
                  { code: 'HERITAGEGIFT', label: 'HERITAGEGIFT (₹100 Off)' },
                ].map((off) => {
                  const isActive = appliedOfferCode.trim().toUpperCase() === off.code;
                  return (
                    <button
                      key={off.code}
                      ...
                      className={`px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold transition-all cursor-pointer border ${
                        isActive
                          ? 'bg-amber-400 text-slate-950 border-amber-500 font-black shadow-xs ring-1 ring-amber-400'
                          : 'bg-white text-slate-700 border-amber-200 hover:bg-amber-100/60'
                      }`}
                    >
                      {isActive ? `✓ ${off.label}` : off.label}
                    </button>
                  );
                })}
              </div>
