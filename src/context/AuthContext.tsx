...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [currentRole, setCurrentRole] = useState<UserRole>(() => {
    try {
      const saved = localStorage.getItem('fabriq_active_role') as UserRole;
  ...
  const [profile, setProfile] = useState<UserProfile | null>(() => {
    try {
      const saved = localStorage.getItem('fabriq_active_role') as UserRole;
      if (saved && ROLE_PROFILES[saved]) return ROLE_PROFILES[saved];
    } catch {}
    return ROLE_PROFILES.customer;
  });
  ...
  const switchRole = (role: UserRole) => {
    setCurrentRole(role);
    setProfile(ROLE_PROFILES[role] || ROLE_PROFILES.customer);
    try {
      localStorage.setItem('fabriq_active_role', role);
    } catch {}
  };
